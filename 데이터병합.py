...
- 동일 사원번호에 대해 부서명이 다를 경우, **가장 마지막 날짜 기준 부서**로 통일합니다.  
...
if caps_file and att_file:
    try:
        # 1) 원본 로드 (사원번호를 문자열로 읽어서 '0029' 유지)
        ...
        # 10) 안전장치: 신규 기록에도 최신 부서 & 정제된 사원명 재덮어쓰기
        ...
        # ————————————— 부서 최종 통합 보정 —————————————
        # merged_df에 datetime 변환
        merged_df['일자_dt'] = pd.to_datetime(merged_df['일자'], errors='coerce')
        # 사원번호별 마지막 부서 재계산
        final_depts = (
            merged_df.sort_values('일자_dt')
                     .groupby('사원번호')['소속부서']
                     .last()
                     .to_dict()
        )
        # 전체 소속부서 덮어쓰기
        merged_df['소속부서'] = merged_df['사원번호'].map(final_depts)
        # 불필요 컬럼 제거
        merged_df.drop(columns=['일자_dt'], inplace=True)
        # ————————————————————————————————————————————————
        ...
        st.success("✅ 병합 완료! 아래에서 최신 부서 통합본을 다운로드하세요.")
        st.download_button(
            label="📥 병합된 근무기록 다운로드",
            data=output,
